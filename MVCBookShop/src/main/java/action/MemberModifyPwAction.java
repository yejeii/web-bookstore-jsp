...
/** 회원 비밀번호 변경 페이지 요청을 처리하는 Action 클래스 */
public class MemberModifyPwAction implements Action {

	@Override
	public ActionForward execute(HttpServletRequest req, HttpServletResponse resp) throws Exception {
		System.out.println(" M.ModifyPw.A : execute() 호출");
		...
		if(session != null) {
			...
			if(s_isLogin) {
				...
				if(s_userId == null) {
					...
					if(s_userId.equals(req.getParameter("userId"))){
						// 세션에 저장된 아이디와 파라미터로 넘어온 아이디가 동일한 경우
						req.setAttribute("m_id", req.getParameter("userId"));
						forward = new ActionForward();
						forward.setPath("/member/modifyPw.jsp");
		...
		System.out.println(" M.ModifyPw.A : execute() 종료");
		return forward;
	}
